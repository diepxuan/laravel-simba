 ...
 * Class AsARFilt3
 *
 * Stored procedure: asARFilt3
 * Mục đích: Lọc danh sách chứng từ công nợ phải thu (AR) loại 3 - header và detail.
 * Procedure thực hiện lọc dữ liệu từ bảng ARPH3 (header) và ARCT3 (detail) dựa trên điều kiện lọc truyền vào.
 ...
 * Điều kiện lọc được xây dựng động qua hai tham số @pKeyPh (điều kiện cho bảng ARPH3) và @pKeyCt (điều kiện cho bảng ARCT3).
 * 
 * Tham số:
 * - @pKeyPh (string, bắt buộc): Điều kiện lọc cho bảng ARPH3 (header). Chuỗi SQL WHERE condition (không bao gồm từ khóa WHERE).
 *   Ví dụ: "ma_cty = 'A01' AND ngay_ct >= '2023-01-01'".
 *   Kiểu dữ liệu: NVARCHAR(1000).
 * - @pKeyCt (string, bắt buộc): Điều kiện lọc cho bảng ARCT3 (detail). Chuỗi SQL WHERE condition (không bao gồm từ khóa WHERE).
 *   Ví dụ: "ma_lo = 'LO01'".
 *   Kiểu dữ liệu: NVARCHAR(1000).
 * 
 * Giá trị mặc định:
 ...
 * - Resultset 2: Các cột: ma_cty, stt_rec, stt_rec0, stt_rec_hd, stt_rec_dh, so_hd, ngay_hd, ma_nt, tt_hd, tt_hd_nt, tien_tt, tien_tt_nt, tien_tt_qd, da_tt, da_tt_nt, du_hd, du_hd_nt, tk, ten_tk, dien_giai, ma_lo, ten_lo, ma_bp, ten_bp, ma_hd, ten_hd, ma_phi, ten_phi, ma_spct, ten_spct.
 * 
 * Logic chi tiết:
 * 1. Xây dựng câu lệnh SQL động cho header, nối với điều kiện WHERE từ @pKeyPh và @pKeyCt.
 *    - Điều kiện: stt_rec phải tồn tại trong ARCT3 thỏa @pKeyCt và đồng thời stt_rec đó phải tồn tại trong ARPH3 thỏa @pKeyPh.
 ...
 * - Các bảng tham chiếu: ARPH3, ARCT3, ardmkh, gldmTk, InDmLo, sidmbp, sidmhd, sidmphi, codmspct, artt.
 * - Module Accounts Receivable (AR) - công nợ phải thu, loại chứng từ 3.
 */
class AsARFilt3
{
    /**
     * Gọi stored procedure asARFilt3
     *
    ...
    {
        return ProcedureCaller::call('asARFilt3', $params);
    }
}